...
public partial class mechine : System.Web.UI.Page
{
    ...
    {
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = "update mechine_management set mechine_name='" + TextBox7.Text + "',brand='" + TextBox8.Text + "',use_ed='" + TextBox9.Text + "',department='" + DropDownList2.SelectedValue+ "',lastserviced='" + TextBox10.Text + "',warranty='" + TextBox11.Text + "' where mechine_id='" + Label1.Text + "'";
        obj.execute(cmd);
        cmd.CommandText = "select*from mechine_management ";
        DataGrid1.DataSource = obj.getData(cmd);
        DataGrid1.DataBind();
